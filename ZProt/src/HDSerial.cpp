...
void _stdcall CHDSerial::Win9xReadHDSerial(WORD *buffer)
{
	for (int i = 0; i < 256; i++)
		buffer[i] = 0;
	_asm
...
char *CHDSerial::WORDToChar(WORD diskdata[256], int firstIndex, int lastIndex)
{
	static char string[1024];
	int index = 0;
	int position = 0;

	//按照高字节在前，低字节在后的顺序将字数组diskdata中内容存入到字符串string中
...
char *CHDSerial::DWORDToChar(DWORD diskdata[256], int firstIndex, int lastIndex)
{
	static char string[1024];
	int index = 0;
	int position = 0;
...
BOOL CHDSerial::WinNTReadIDEHDSerial(DWORD * buffer)
{
...
BOOL CHDSerial::WinNTReadSCSIHDSerial(DWORD *buffer)
{
	int controller = 0;
	...
	buffer[0] = L'\n';
	wsprintf(driveName, L"\\\\.\\Scsi%d:", controller);
...
BOOL CHDSerial::WinNTGetIDEHDInfo(HANDLE hPhysicalDriveIOCTL,
